...
namespace DirectShowLib.Utils
{
    ...
    public sealed class FilterGraphTools
    {
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public static IBaseFilter GetConnectedFilter(IBaseFilter source, int iIndex)
        {
            IBaseFilter retFilter = null;
            int hr = 0;

            IPin sPin = DsFindPin.ByConnectionStatus(source, PinConnectedStatus.Connected, iIndex);
            IPin dPin = null;

            try
            {
                hr = sPin.ConnectedTo(out dPin);
                DsError.ThrowExceptionForHR(hr);

                retFilter = GetFilterFromPin(dPin);
            }
            finally
            {
                if (dPin != null)
                    Marshal.ReleaseComObject(dPin);
                dPin = null;

                if (sPin != null)
                    Marshal.ReleaseComObject(sPin);
                sPin = null;
            }
        ...
        [SecurityPermission(SecurityAction.LinkDemand, UnmanagedCode = true)]
        public static IBaseFilter GetConnectedFilter(IBaseFilter source, PinDirection vDir, int iIndex)
        {
            IBaseFilter retFilter = null;
            int hr = 0;

            IPin sPin = DsFindPin.ByDirection(source, vDir, iIndex);
            IPin dPin = null;
